...
class Tail(base.Base):
    ...
    def set_controller_shape():
        ...
        sphere_ctrl = cmds.rename(cmds.listRelatives(sphere, p=True), 'TailIK_tempShape')
    ...
    def build_guide(self):
        ...
        for i in range(self.segment):
            ...
            if i is 0:
                ...
                cmds.move(self.start_pos[0], self.start_pos[1], self.start_pos[2], tail, absolute=True)
                cmds.scale(self.scale, self.scale, self.scale, tail)
            # tail locator parent to the previous locator
            else:
                cmds.parent(tail, self.loc_list[i-1], relative=True)
                # move tail locator along -y axis
                cmds.move(0, -self.interval, 0, tail, relative=True)
    ...
    def place_controller(self):
        ...
        tail_pos = cmds.xform(self.jnt_list[0], q=True, t=True, ws=True)
        cmds.move(tail_pos[0], tail_pos[1], tail_pos[2]-1, self.master_ctrl)
        cmds.addAttr(self.master_ctrl, longName='FK_IK', attributeType='double', defaultValue=1, minValue=0, maxValue=1, keyable=True)

        # IK and FK control has the same setup
        for i in range(self.segment):
            cmds.duplicate('TailIK_tempShape', name=self.ik_ctrl_list[i])
            cmds.group(em=True, name=self.ik_offset_list[i])
            cmds.duplicate('TailFK_tempShape', name=self.fk_ctrl_list[i])
            cmds.group(em=True, name=self.fk_offset_list[i])

        for i, tail in enumerate(self.jnt_list):
            tail_pos = cmds.xform(tail, q=True, t=True, ws=True)
            tail_rot = cmds.xform(tail, q=True, ro=True, ws=True)
        ...
        for i in range(self.segment):
            cmds.parent(self.ik_ctrl_list[i], self.ik_offset_list[i], relative=True)
            cmds.parent(self.fk_ctrl_list[i], self.fk_offset_list[i], relative=True)
            if i != 0:
    ...
    def build_ik(self):
        ...
        cmds.inheritTransform(tail_curve, off=True)

        cvs = cmds.ls(self.ik_curve+'.cv[0:]', fl=True)
