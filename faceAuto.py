...
class FaceJoint():
    ...
    def createFaceWindow(self, void):
        cmds.window("Facial AutoRig")
        cmds.rowColumnLayout(nc=2)
        cmds.button(l="Create Face Locators", w=200, c=self.Locators)
        cmds.button(l="Mirror Face Locators", w=200, c=self.Mirror)
        cmds.button(l="Delete Locators", w=200, c=self.DeleteLoc)
        cmds.button(l="Create Face Joints", w=200, c=self.FaceJoint)
        cmds.showWindow()
    ...
    def EyeLocators(self):
        if(cmds.objExists('L_Eye')):
            ...
            for side in sides:
                ...
                cmds.scale(1, 1, 1, eyeCenterLoc)
                eyePos = cmds.xform('%s_Eye.rotatePivot' % side, q=True, t=True, ws=True)
                cmds.move(eyePos[0], eyePos[1], eyePos[2], eyeCenterLoc)

                eyeAimLoc = cmds.spaceLocator(n='Loc_Face_'+side+'_EyeAim')
                cmds.scale(1, 1, 1, eyeAimLoc)
                cmds.move(eyePos[0], eyePos[1], eyePos[2]+10*scaleFac, eyeAimLoc)

                upperLid = cmds.curve(p=eyeUpLidCurve, n="CV_%s_UpperEyeLid" % side)
                cmds.scale(2.5, 2.5, 2.5, upperLid)
                cmds.move(eyePos[0]-2.2*scaleFac*sideMult, eyePos[1], eyePos[2]+6*scaleFac, upperLid)

                lowerLid = cmds.curve(p=eyeLowLidCurve, n="CV_%s_LowerEyeLid" % side)
                cmds.scale(2.5, 2.5, 2.5, lowerLid)
                cmds.move(eyePos[0]-2.2*scaleFac*sideMult, eyePos[1]-1.5*scaleFac, eyePos[2]+6*scaleFac, lowerLid)

                eyeBrow = cmds.curve(p=eyeBrowCurve, n="CV_%s_EyeBrow" % side)
                cmds.scale(4, 3, 2.5, eyeBrow)
                cmds.move(eyePos[0]-3.5*scaleFac*sideMult, eyePos[1]+2.5*scaleFac, eyePos[2]+8*scaleFac, eyeBrow)

                foreHeadBrow = cmds.curve(p=eyeBrowCurve, n="CV_%s_ForeHeadBrow" % side)
                cmds.scale(5, 4, 2.5, foreHeadBrow)
                cmds.move(eyePos[0]-3.5*scaleFac*sideMult, eyePos[1]+7*scaleFac, eyePos[2]+8*scaleFac, foreHeadBrow)

                cheekLoc = cmds.spaceLocator(name='Loc_Face_%s_Cheek' % side)
                cmds.move(eyePos[0]+4*scaleFac*sideMult, eyePos[1]-6*scaleFac, eyePos[2]+7*scaleFac, cheekLoc)
            ...
            cmds.confirmDialog(title="Eyes missing", message="The eyes could not be found", button=['OK'])
    ...
    def AddLocators(self):
        ...
        cmds.move(0, 371*self.scale, 8*self.scale, headLoc)

        centerLoc = cmds.spaceLocator(n='Loc_Face_Center')
        cmds.move(0, 336*self.scale, -3.5*self.scale, centerLoc)

        jawEndLoc = cmds.spaceLocator(n='Loc_Face_JawEnd')
        cmds.move(0, 325*self.scale, 0, jawEndLoc)
    ...
    def FaceJoint(self, void):
        ...
        '''otherLocs = cmds.ls('Jaw_Loc', 'Face_Loc', 'Center_Loc')
        for loc in otherLocs:
            locPos = cmds.xform(loc, q=True, t=True, ws=True)
            cmds.select(clear=True)
            name= loc.split('_Loc')[0]
            jnt = cmds.joint(p=locPos, name='Jnt_Face_' + name)
            cmds.parent(jnt, jointGrp)'''
        self.ParentJoint()
